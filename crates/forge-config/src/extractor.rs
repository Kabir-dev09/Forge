...
use forge_core::config_registry::{ForgeConfig, CursorStyle};
...
fn get_color(table: &Table, key: &str) -> Option<Color> {
    get_string(table, key).and_then(|s| {
        ...
        if parsed.is_none() {
            tracing::warn!("Failed to parse color '{}' for key '{}'. Expected hex format (e.g., #RRGGBB).", s, key);
        }
        parsed
    })
}
...
pub fn extract_config(root: Table, config: &mut ForgeConfig) {
    ...
    if let Ok(Value::Table(font_t)) = root.get::<_, Value>("font") {
        if let Some(f) = get_string(&font_t, "family") { config.font.family = f; }
        if let Some(s) = get_f32(&font_t, "size") { config.font.size = s; }
        if let Some(f) = get_string(&font_t, "bold_family") { config.font.bold_family = Some(f); }
        if let Some(f) = get_string(&font_t, "italic_family") { config.font.italic_family = Some(f); }
        if let Some(l) = get_bool(&font_t, "ligatures") { config.font.ligatures = l; }
        if let Some(n) = get_bool(&font_t, "nerd_fonts") { config.font.nerd_fonts = n; }
    }

    // Window
    if let Ok(Value::Table(win_t)) = root.get::<_, Value>("window") {
        if let Some(w) = get_u32(&win_t, "width") { config.window.width = w; }
        if let Some(h) = get_u32(&win_t, "height") { config.window.height = h; }
        if let Some(o) = get_f32(&win_t, "opacity") { config.window.opacity = o; }
        ...
        match win_t.get::<_, Value>("padding") {
            ...
            Ok(Value::Table(pad_t)) => {
                ...
                if let Some(p) = get_u32(&pad_t, "top") { config.window.padding.top = p; }
                if let Some(p) = get_u32(&pad_t, "bottom") { config.window.padding.bottom = p; }
                if let Some(p) = get_u32(&pad_t, "left") { config.window.padding.left = p; }
                if let Some(p) = get_u32(&pad_t, "right") { config.window.padding.right = p; }
            }
            _ => {}
        }
        
        if let Some(t) = get_string(&win_t, "title") { config.window.title = t; }
        if let Some(d) = get_bool(&win_t, "decorations") { config.window.decorations = d; }
        if let Some(s) = get_string(&win_t, "padding_balance") {
            match s.to_lowercase().as_str() {
                "fill" => config.window.padding_balance = forge_core::config_registry::PaddingBalance::Fill,
                "center" => config.window.padding_balance = forge_core::config_registry::PaddingBalance::Center,
                _ => {}
            }
        }
    }
    ...
    if let Ok(Value::Table(theme_t)) = root.get::<_, Value>("theme") {
        if let Some(c) = get_color(&theme_t, "background") { config.theme.background = c; }
        if let Some(c) = get_color(&theme_t, "foreground") { config.theme.foreground = c; }
        if let Some(c) = get_color(&theme_t, "cursor_color") { config.theme.cursor_color = c; }
        if let Some(c) = get_color(&theme_t, "selection_bg") { config.theme.selection_bg = c; }
    ...
    if let Ok(Value::Table(shell_t)) = root.get::<_, Value>("shell") {
        if let Some(p) = get_string(&shell_t, "program") { config.shell.program = p; }
        ...
        if let Ok(Value::Table(args_t)) = shell_t.get::<_, Value>("args") {
            ...
            for i in 1.. {
                match args_t.get::<_, Value>(i) {
                    Ok(Value::String(s)) => {
                        if let Ok(s_str) = s.to_str() { args.push(s_str.to_string()); }
                    }
                    _ => break,
                }
            }
            if !args.is_empty() { config.shell.args = args; }
        }
        ...
        if let Ok(Value::Table(env_t)) = shell_t.get::<_, Value>("env") {
            ...
            if !env.is_empty() { config.shell.env = env; }
        }
    }
    ...
    if let Ok(Value::Table(cursor_t)) = root.get::<_, Value>("cursor") {
        ...
        if let Some(b) = get_bool(&cursor_t, "blink") { config.cursor.blink = b; }
        if let Some(r) = get_u32(&cursor_t, "blink_rate_ms") { config.cursor.blink_rate_ms = r; }
    }

    // Scrollback
    if let Ok(Value::Table(scroll_t)) = root.get::<_, Value>("scrollback") {
        if let Some(l) = get_usize(&scroll_t, "lines") { config.scrollback.lines = l; }
        if let Some(s) = get_bool(&scroll_t, "smooth_scroll") { config.scrollback.smooth_scroll = s; }
        if let Some(m) = get_f32(&scroll_t, "scroll_multiplier") { config.scrollback.scroll_multiplier = m; }
    }

    // Behavior
    if let Ok(Value::Table(behavior_t)) = root.get::<_, Value>("behavior") {
        if let Some(c) = get_bool(&behavior_t, "copy_on_select") { config.behavior.copy_on_select = c; }
        if let Some(d) = get_bool(&behavior_t, "disable_default_keybindings") { config.behavior.disable_default_keybindings = d; }
...
mod tests {
    ...
    fn test_extract_config() {
        ...
        assert_eq!(config.theme.background, Color { r: 26, g: 27, b: 38, a: 255 });
        assert_eq!(config.theme.foreground, Color { r: 192, g: 202, b: 245, a: 170 });
        assert_eq!(config.theme.ansi_colors[0], Color { r: 17, g: 17, b: 17, a: 255 });
        assert_eq!(config.theme.ansi_colors[1], Color { r: 34, g: 34, b: 34, a: 255 });
        // `cursor_color` should be the default, not "red"
        assert_eq!(config.theme.cursor_color, ForgeConfig::default().theme.cursor_color);

        assert_eq!(config.shell.program, "/bin/zsh");
        assert_eq!(config.shell.args, vec!["-l".to_string(), "-i".to_string()]);
        assert_eq!(config.shell.env, vec![("FOO".to_string(), "bar".to_string())]);
