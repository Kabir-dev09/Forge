...
impl<'a> Perform for TerminalPerformer<'a> {
    fn print(&mut self, c: char) {
        if c >= '\x20' {
            self.buffer.write_grapheme(&c.to_string());
        }
    }
    ...
    fn execute(&mut self, byte: u8) {
        match byte {
            ...
            0x09 => {
                ...
                self.buffer.move_cursor_to(self.buffer.cursor.row, next_tab.min(self.buffer.cols().saturating_sub(1)));
            }
    ...
    fn csi_dispatch(&mut self, params: &Params, intermediates: &[u8], _ignore: bool, action: char) {
        ...
        match action {
            ...
            'G' => self.buffer.move_cursor_to(self.buffer.cursor.row, (p0 - 1).max(0) as usize),
            'H' | 'f' => self.buffer.move_cursor_to((p0 - 1).max(0) as usize, (p1 - 1).max(0) as usize),
            ...
            'r' => {
                ...
                let bottom = if bottom == 0 { self.buffer.rows().saturating_sub(1) } else { bottom.min(self.buffer.rows().saturating_sub(1)) };
            ...
            'q' => {
                if intermediates.first() == Some(&b' ') {
                    ...
                    match p0 {
                        0 | 1 => {
                            self.buffer.cursor_style_override = Some(forge_core::config_registry::CursorStyle::Block);
                        ...
                        3 => {
                            self.buffer.cursor_style_override = Some(forge_core::config_registry::CursorStyle::Underline);
                            self.buffer.cursor_blink_override = Some(true);
                        }
                        4 => {
                            self.buffer.cursor_style_override = Some(forge_core::config_registry::CursorStyle::Underline);
                        ...
                        5 => {
                            self.buffer.cursor_style_override = Some(forge_core::config_registry::CursorStyle::Beam);
                            self.buffer.cursor_blink_override = Some(true);
                        }
                        6 => {
                            self.buffer.cursor_style_override = Some(forge_core::config_registry::CursorStyle::Beam);
    ...
    fn esc_dispatch(&mut self, _intermediates: &[u8], _ignore: bool, byte: u8) {
        match byte {
            b'7' => self.buffer.saved_cursor = Some(self.buffer.cursor),
            b'8' => {
            ...
            _ => tracing::trace!("Unhandled ESC: 0x{:02X}", byte),
        }
    }
}

fn get_param_or(params: &Params, index: usize, default: u16) -> u16 {
    params.iter().nth(index).and_then(|p| p.iter().next().copied()).filter(|&v| v != 0).unwrap_or(default)
}
...
fn handle_sgr(params: &Params, buffer: &mut ScreenBuffer) {
    ...
    while i < flat.len() {
        match flat[i] {
            ...
            100..=107 => buffer.current_bg = ansi_256_color(flat[i] as u8 - 100 + 8, &buffer.palette),
            38 => {
    ...
    if intermediates.contains(&b'?') {
        for param in params {
            if param == [1] {
                ...
                tracing::trace!("Application cursor keys: {}", buffer.application_cursor_keys);
...
impl VteProcessor {
    pub fn new() -> Self {
        VteProcessor { parser: Parser::new() }
    }
}
...
impl VteProcessor {
    ...
    pub fn process(&mut self, data: &[u8], buffer: &mut ScreenBuffer) -> Vec<u8> {
        let mut performer = TerminalPerformer { buffer, responses: Vec::new() };
        for &byte in data {
...
mod tests {
    ...
    fn test_sgr_colors() {
        ...
        assert_eq!(buf.current_fg, Color { r: 100, g: 200, b: 50, a: 255 });

        processor.process(b"\x1b[0m", &mut buf);
        assert_eq!(buf.current_fg, Color { r: 192, g: 202, b: 245, a: 255 });
    }
    ...
    fn test_cursor_movement_edge_cases() {
        let mut processor = VteProcessor::new();
        let mut buf = ScreenBuffer::new(20, 20, 100, forge_core::color::Color { r: 192, g: 202, b: 245, a: 255 }, forge_core::color::Color { r: 30, g: 30, b: 46, a: 255 });
    ...
    fn test_sgr_256_colors() {
        ...
        assert_eq!(buf.current_fg, Color { r: 0, g: 0, b: 0, a: 255 });
    ...
    fn test_sgr_colon_separated() {
        ...
        assert_eq!(buf.current_fg, Color { r: 10, g: 20, b: 30, a: 255 });
    }
}
